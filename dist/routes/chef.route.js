"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
var express_1 = require("express");
var error_middlewear_1 = require("../middlewares/error.middlewear");
var validate_middlewear_1 = require("../middlewares/validate.middlewear");
var chef_validator_1 = require("../validator/chef.validator");
var chef_controller_1 = require("../controllers/chef.controller");
var multer_config_1 = require("../config/multer.config");
var router = (0, express_1.Router)();
router.post('/create', multer_config_1.uploadDisk.single('file'), (0, error_middlewear_1.asyncHandelError)(chef_controller_1.createChef));
router.put('/:id/updateTextDataChefById', (0, validate_middlewear_1.validateBody)(chef_validator_1.chefValidate), (0, error_middlewear_1.asyncHandelError)(chef_controller_1.updateTextDataChefById));
router.put('/:id/updateContentImage', multer_config_1.uploadMemory.single('file'), (0, error_middlewear_1.asyncHandelError)(chef_controller_1.updateContentImageVPS));
router.get('/:id/get', (0, error_middlewear_1.asyncHandelError)(chef_controller_1.getByIdChef));
router.get('/getAll/:page/:size', (0, error_middlewear_1.asyncHandelError)(chef_controller_1.getAllChef));
router.delete('/:id/delete', (0, error_middlewear_1.asyncHandelError)(chef_controller_1.deleteChefById));
exports.default = router;
